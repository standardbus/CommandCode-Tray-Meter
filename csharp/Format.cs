...
namespace CommandCodeMonitor
{
    ...
    internal static class Format
    {
        ...
        /// <summary>`"2"`, `"2.5"`, `"8.45"` - two decimals at most, dotted separator.</summary>
        ...
        public static string TokenCount(double value)
        {
            ...
            if (value >= 1e9) return (value / 1e9).ToString("0.00", Culture) + " B";
            if (value >= 1e6) return (value / 1e6).ToString("0.0", Culture) + " M";
            if (value >= 1e3) return (value / 1e3).ToString("0.0", Culture) + " K";
        ...
        /// <summary>`"3h 12m"`, `"2d 4h"`, `"45m"`, `"<1m"`.</summary>
        public static string Delta(TimeSpan span)
        {
            if (span.TotalMilliseconds <= 0) return "<1m";
